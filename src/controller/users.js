    ...
    try {
      ...
      if (result) {
        const verifUrl = `http://${Host}:${Port}/users/${req.body.email_user}/${otp}`
                const sendEmail =  email(data.email_user,otp,verifUrl,data.fullname_user)
                // eslint-disable-next-line eqeqeq
                if(sendEmail == 'email not sent!'){
                    return response(res, 404, false, null, "register fail")
                }
                response(res, 200, true,{email_user:data.email_user},"register success please check your email")
      }
    } catch (err) {
...
otp: async(req,res)=>{
  console.log('email',req.params.email_user)
  console.log('password',req.params.otp)
  const {rows:[users]} = await findEmail(req.params.email_user)
    if(!users){
